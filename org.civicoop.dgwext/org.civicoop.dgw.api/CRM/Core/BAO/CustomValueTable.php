...
class CRM_Core_BAO_CustomValueTable {
   ...
   * @param boolean     $onlyActiveFields Set to true to only return active fields. If false also non-active fields will be returned. 
   *
   ...
   * @static
   */
  public static function &getEntityValues($entityID, $entityType = NULL, $fieldIDs = NULL, $formatMultiRecordField = FALSE, $onlyActiveFields = true) {
    if (!$entityID) {
...
";
    if ($onlyActiveFields) {
		$query .= " AND    cg.is_active = 1 AND cf.is_active = 1";
    }
	$query .= " AND ".$cond;
...
";
  ...
  static function &getValues(&$params) {
    ...
    $onlyActiveFields = (!isset($params['onlyActiveFields']) || $params['onlyActiveFields'] ? true : false);  
