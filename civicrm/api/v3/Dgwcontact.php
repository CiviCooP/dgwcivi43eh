...
function civicrm_dgwcontact_addressget($inparms) {
    ...
    if (!isset($inparms['address_id']) || empty($inparms['address_id'])) {
        /*
         * use civicrm_location_get for the contact to retrieve addresses
         */
        $civiparms = array(
            "contact_id"    =>  $contact_id,
            "version"       =>  "3.0");
        ...
        if (civicrm_error($civires)) {
             ...
             * - read all items within a location. This can contain an address,
             *   emails or phones.
             * - If the item is not an array we are on the address level
             */
            $i = 1;
            foreach ($civires as $location) {
                foreach ($location as $item) {
                    if (is_array($item)) {
                        if (isset($item['street_address'])) {
                            $outparms[$i]['contact_id'] = $item['contact_id'];
                            $outparms[$i]['address_id'] = $item['id'];
                            $outparms[$i]['street_address'] =
                                $item['street_address'];
                            if (isset($item['location_type_id'])) {
                                $outparms[$i]['location_type'] = strtolower(
                                getLocationType($item['location_type_id']));
                           }
                            if (isset($item['is_primary'])) {
                            ...
                            if (isset($item['street_name'])) {
                                $outparms[$i]['street_name'] = $item['street_name'];
                            }
                            if (isset($item['street_number'])) {
                                $outparms[$i]['street_number'] =
                                    $item['street_number'];
                            }
                            if (isset($item['street_number_suffix'])) {
                                $outparms[$i]['street_suffix'] =
                                    $item['street_number_suffix'];
                            }
                            /*
                             * issue 220: if street unit is entered, use it
                             */
                            if (isset($item['street_unit']) && 
								!empty($item['street_unit'])) {
								$outparms[$i]['street_suffix'] .= " ".
									$item['street_unit'];
							}	
                            if (isset($item['postal_code'])) {
                                $outparms[$i]['postal_code'] = $item['postal_code'];
                            }
                            if (isset($item['city'])) {
                                $outparms[$i]['city'] = $item['city'];
                            }
                            if (isset($item['country_id'])) {
                                $outparms[$i]['country_id'] = $item['country_id'];
                            ...
                            /*
                             * Make it possible to pass on start date for address.
                             * This is not part of CiviCRM yet, but might be in
                             * future and will probably be required for De Goede
                             * Woning anyway. So already in place in API even if
                             * not functional yet
        ...
        /*
         *
         * retrieve specific address if address_id is used
         */
        $address_id = $inparms['address_id'];
        $query = "SELECT * FROM civicrm_address WHERE id = $address_id";
        $daoAddress = CRM_Core_DAO::executeQuery($query);
        $outparms[0]['record_count'] = 0;
            while ($daoAddress->fetch()) {
                $outparms[1]['contact_id'] = $daoAddress->contact_id;
                $outparms[1]['address_id'] = $daoAddress->id;
                $outparms[1]['location_type'] =
                    getLocationType($daoAddress->location_type_id);
                $outparms[1]['location_type'] = strtolower(
                        $outparms[1]['location_type']);
                $outparms[1]['is_primary'] = $daoAddress->is_primary;
                $outparms[1]['street_address'] = $daoAddress->street_address;
                $outparms[1]['street_name'] = $daoAddress->street_name;
                $outparms[1]['street_number'] = $daoAddress->street_number;
                $outparms[1]['street_suffix'] = $daoAddress->street_number_suffix;
                /*
                 * issue 220: if street unit is entered, use it
                 */
                if (!empty($daoAddress->street_unit)) {
					$outparms[1]['street_suffix'] .= " ".$daoAddress->street_unit;
				} 
                $outparms[1]['postal_code'] = $daoAddress->postal_code;
                $outparms[1]['city'] = $daoAddress->city;
                $outparms[1]['country_id'] = $daoAddress->country_id;
                $outparms[1]['country'] = CRM_Core_PseudoConstant::country(
                    $daoAddress->country_id);
                $outparms[1]['start_date'] = date("Y-m-d");
                $outparms[1]['end_date'] = "";
                $outparms[0]['record_count'] = 1;
         }
