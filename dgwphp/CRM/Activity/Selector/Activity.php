...
/*
 ...
 | Project	:   CiviCRM Maintenance                               |
 | Marker	:   DGW16                                             |
 | Date         :   19 July 2011	                   	      |
 | Description  :   in function actionLinks: cid becomes the first    |
 |                  target contact ID if there are any                |
 |                                                                    |
 | Author       :   Erik Hommel (erik.hommel@civicoop.org)            |
 | Marker       :   incident 14 01 13 003 / DGW19                     |
 | Date         :   13 March 2013                                     |
 | Description  :   No edit or delete options for protected           |
 |                  activity types                                    |
 +--------------------------------------------------------------------+
*/
...
class CRM_Activity_Selector_Activity extends CRM_Core_Selector_Base implements CRM_Core_Selector_API 
{
    ...
    {
        /*
         * DGW16: if there are any target contacts for the activity in
         * question, save the first target in field $actCid
         */ 
        if ($this->_context == "home") {
        ...
        /*
         * DGW16 in build of all links, use $actCid if not empty
         */
        ...
        if ( $activityTypeId == $activityTypeIds['Event Registration'] )  { // event registration
            ...
            if ( isset( $actCid ) && !empty( $actCid ) ) {
                $qsView = "action=view&reset=1&id={$sourceRecordId}&cid={$actCid}&context=%%cxt%%{$extraParams}";
            } else {
            ...
            if ( isset( $actCid ) && !empty( $actCid ) ) {
                $qsView   = "atype={$activityTypeId}&action=view&reset=1&id=%%id%%&cid={$actCid}&context=%%cxt%%{$extraParams}";
            } else {
            ...
            if ( isset( $actCid ) && !empty( $actCid ) ) {
                $qsView   = "atype={$activityTypeId}&action=view&reset=1&id=%%id%%&cid={$actCid}&context=%%cxt%%{$extraParams}";
                $qsUpdate = "atype={$activityTypeId}&action=update&reset=1&id=%%id%%&cid={$actCid}&context=%%cxt%%{$extraParams}";
            } else {
                $qsView   = "atype={$activityTypeId}&action=view&reset=1&id=%%id%%&cid=%%cid%%&context=%%cxt%%{$extraParams}";
        ...
        if ( isset( $actCid ) && !empty( $actCid ) ) {
            $qsDelete  = "atype={$activityTypeId}&action=delete&reset=1&id=%%id%%&cid={$actCid}&context=%%cxt%%{$extraParams}";
        } else {
	...
	if ( $this->_context == 'case' ) {
        ...
        /*
         * incident 14 01 13 003 / DGW19 only give edit and delete options for
         * special activities if current user authorised (member of group)
         */
        $typeActWijk = 109;
        $typeCaseWijk = 110;
        $typeDirBest = 118;
        if ( $activityTypeId == $typeActWijk || $activityTypeId == $typeCaseWijk || $activityTypeId == $typeDirBest ) {
            if ( !$session ) {
        ...
        if ( $activityTypeId == $typeActWijk || $activityTypeId == $typeCaseWijk ) {
        ...
        /*
         * DGW19 / incident 14 01 13 003 - only if showUpdate
         */
        if ( $showUpdate ) {
            ...
            if ( CRM_Case_BAO_Case::checkPermission( $activityId, 'File On Case', $activityTypeId ) ) {
                self::$_actionLinks = self::$_actionLinks +  array ( CRM_Core_Action::ADD =>
                     array( 
                         // DGW19 / incident 14 01 13 003 - change text
                           //'name'     => ts('File On Case'),
                           'name'     => 'Sla op bij dossier',
