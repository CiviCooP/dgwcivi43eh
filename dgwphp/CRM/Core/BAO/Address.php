...
| Marker : DGW8                                                      |
| Date : 2 Feb 2011 (reapplied to 3.3.5 on 2-3-11)                   |
| Description : Street parsing in Dutch format                       |
+--------------------------------------------------------------------+
| Incident 19 06 12 002                                              |
| Street parsing is incorrect if first part of street_name is a      |
| number                                                             |
| Example : 1e Wormenseweg 245 should lead to:                       |
|   street_name = 1e Wormenseweg                                     |
|   street_number = 245                                              |
| but actual result is:                                              |
|   street_name = 1e                                                 |
|   street_number = 1                                                |
|   street_number_suffix = e Wormen                                  |
| Author	:	Erik Hommel (hommel@ee-atwork.nl)                    |
| Date		:	21 June 2012                                         |
| Solution	:	Rebuild function                                     |
+--------------------------------------------------------------------+ 
*/
...
*/
        /*
* DGW8 added nl_NL as supported locale
*/
...
// $parseFields['street_name'] = $streetAddress;
        ...
        /*
		 * DGW8: street parsing in Dutch format
		 */
		$foundNumber = false;
        ...
        foreach ( $parts as $key => $part ) {
			...
			if ( is_numeric( $part ) ) {
				/*
				 * if key =1, add to street_name
				 */
				if ( $key == 0 ) {
					$parseFields['street_name'] .= $part;
				/*
				 * else add to street_number if not found, else
				 * add to suffix
				 */	
				} else {
					if ( $foundNumber == false ) {
						$parseFields['street_number'] .= $part;
						$foundNumber = true;
					} else {
						$parseFields['street_number_suffix'] .= " ".$part;
					}
				}
				/*
				 * if not numeric
				...
				/*
				 * if no foundNumber, add to street_name
				 */
				if ( $foundNumber == false ) {
					/*
					...
					if ( $key == 0 ) {
						...
						/*
						 * if part has numbers first and non-numbers later
						 * put number into street_number and rest in suffix
						 * and set foundNumber = true
						 */
						 $length = strlen( $part );
						 if ( is_numeric( substr( $part, 0, 1 ) ) ) {
							 for ( $i=0; $i<$length; $i++ ) {
								 if ( is_numeric( substr( $part, $i, 1 ) ) ) {
									 $parseFields['street_number'] .= substr( $part, $i, 1 );
									 $foundNumber = true;
								 } else {
									 $parseFields['street_number_suffix'] .= substr( $part, $i, 1 );
								 }
							 }
						 } else {
							$parseFields['street_name'] .= " ".$part;
		...
		$parseFields['street_name'] = trim( $parseFields['street_name'] );
		$parseFields['street_number_suffix'] = trim( $parseFields['street_number_suffix'] );
        return $parseFields;
    }
