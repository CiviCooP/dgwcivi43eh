...
WHERE  inst.report_id = %1";
    ...
    static function export2csv( &$form, &$rows ) {
        ...
        //Output the headers.
        //DGW25 replace hard coded ',' with sytemvalue (fixed in V4)
        echo implode($config->fieldSeparator, $headers) . "\n";
        //end DGW25
        ...
        foreach ( $rows as $row ) {
            ...
            //Output the data row.
			//DGW25 replace hard coded ',' with systemvalue (fixed in V4)
            echo implode($config->fieldSeparator, $displayRows) . "\n";
        }
