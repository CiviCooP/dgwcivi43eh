...
/*
 ...
 | Project      :   Implementation CiviCRM                            |
 | Marker       :   DGW17                                             |
 | Date         :   14 sept 2011                                      |
 | Description	:   Sort dashlet My Cases on case type, scheduled date|
 |                                                                    |
 | Marker       :   DGW23                                             |
 | Author       :   Erik Hommel (hommel@ee-atwork.nl)                 |
 | Date         :   25 jan 2012                                       |
 | Description  :   Add address to activity mail to target            |
 |                                                                    |
 | Marker       :   DGW19 / incident 14 01 13 003                     |
 | Author       :   Erik Hommel (erik.hommel@civicoop.org)            |
 | Date         :   11 March 2013                                     |
 | Desscription :   Do not email details and attachments to act.      |
 |                  assignee for special types                        |
 +--------------------------------------------------------------------+
*/
...
";
    ...
    function getCaseActivityQuery( $type = 'upcoming', $userID = null, $condition = null, $isDeleted = 0 ) 
    {
		...
		// DGW17 add $type 'type_up'
        if ( $type == 'upcoming' || $type == 'type_up' ) {
		...
		// DGW17 add $type 'type_up' and add case_type_name for sorting
        } else if ( $type == 'type_up' ) {
			$query .= " ORDER BY case_type_name ASC, case_scheduled_activity_date ASC ";
		}
...
GROUP BY cc.id';
    ...
    {   
        ...
        foreach ( $contacts as $mail => $info ) {
            ...
            /*
             * DGW19 and incident 14 03 13 001 - change details and
             * remove attachments if special sensitive activity type
             */
            if ( $activityParams['activity_type_id'] == 3 ) {
                /*
                 * activityParams['source_record_id'] contains previous activity
                 * id. Retrieve using API
                 */
                if ( isset( $activityParams['source_record_id'] ) ) {
                    require_once 'api/v2/Activity.php';
                    $prevActId = $activityParams['source_record_id'];
                    $prevActParams = array( 'activity_id' => $prevActId );
                    $prevActApi = civicrm_activity_get( $prevActParams );
                    $checUserAut = false;
                    if ( !civicrm_error( $prevActApi ) ) {
                        $prevActTypeId = $prevActApi['result']['activity_type_id'];
                        if ( $prevActTypeId == 109 || $prevActTypeId == 110 || $prevActTypeId == 118 ) {
                            $checkUserAut = true;
                        }
                    }
                    /*
                     * if required, check if mail recipient is authorised to view
                     * details
                     */
                    if ( $checkUserAut ) {
                        $checkParams = array(
                            'user_id'       =>  $info['contact_id'],
                            'is_wijk'       =>  1,
                            'is_dirbest'    =>  1
                            
                        );
                        require_once 'CRM/Utils/DgwUtils.php';
                        $checkUser = CRM_Utils_DgwUtils::getGroupsCurrentUser( $checkParams );
                        if ( $checkUser['is_error'] == 0 ) {
                            $userConsulent = $checkUser['wijk'];
                        ...
                        $rplTxt = null;
                        $rplSubj = null;
                        if ( $prevActTypeId == 118 && !$userDirBest ) {
                            $rplSubj = "Afgeschermd gespreksverslag Dir/Best";
                            $rplTxt = "De details en bijlagen van deze activiteit zijn afgeschermd, ";
                            $rplTxt .= "neem contact op met de directeur/bestuurder voor meer info.";
                            $attachments = array();
                        }
                        if ( $prevActTypeId == 109 || $prevActTypeId == 110 ) {
                            if ( !$userConsulent ) {
                                $rplSubj = "Afgeschermd gespreksverslag Wijk en Ontwikkeling";
                                $rplTxt = "De details en bijlagen van deze activiteit zijn afgeschermd, ";
                                $rplTxt .= "neem contact op met de Consulent Wijk en";
                                $rplTxt .= "Ontwikkeling voor meer info.";
                                $attachments = array();
                            }
                        }
                        if ( !empty( $rplTxt ) ) {
                            foreach ( $tplParams['activity']['fields'] as$keyMail => $mailFields ) {
                                if ( $mailFields['label'] == 'Details' ) {
                                    $tplParams['activity']['fields'][$keyMail]['value'] = "<p>".$rplTxt."</p>";
                                }
                                if ( $mailFields['label'] == "Subject") {
                                    $tplParams['activity']['fields'][$keyMail]['value'] = $rplSubj;
                                }
                            }
                        }
                    }
            ...
            /*
             * end DGW19 / incident 14 01 13 003
             */
