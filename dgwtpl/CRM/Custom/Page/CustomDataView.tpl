	...
	<table class="no-border">
	    ...
	    {if $editOwnCustomData or ($showEdit and $editCustomData and $groupId)}
                ...
                {if $cd_edit.title ne "Koopovereenkomst" and $cd_edit.title ne "Huurovereenkomst" and $cd_edit.title ne "Huurovereenkomst (org)"}
                    <tr>
                        <td>
                            <a href="{crmURL p="civicrm/contact/view/cd/edit" q="tableId=`$contactId`&cid=`$contactId`&groupId=`$groupId`&action=update&reset=1"}" class="button" style="margin-left: 6px;"><span><div class="icon edit-icon"></div>{ts 1=$cd_edit.title}Edit %1{/ts}</span></a>{$cd_edit.title}<br/><br/>
                        </td>
                    </tr>
                {/if}
