...
{else}
    ...
    {literal}
        <script type="text/javascript">var target_contact = assignee_contact = '';
    {/literal}
   ...
   {literal}
    ...
    // DGW14 added var tokenDataUrl_assignee
    ...
    // DGW14 changed tokenDataUrl to tokenDataUrl_assignee
    ...
    {elseif $action eq 1 or $action eq 2  or $action eq 4 or $context eq 'search' or $context eq 'smog'}
        ...
        <table class="{if $action eq 4}crm-info-panel{else}form-layout{/if}">
		     ...
		     {else}
                ...
                <td class="view-value" style="white-space: normal">
                    {foreach from=$target_contact key=id item=name}
                        ...
                        {crmAPI var="naw" entity="Contact" action="get" version="2" contact_id=$id }
	     ...
	     {/if}
             ...
             {* {if $action neq 4} *}
             {* Don't include "Schedule Follow-up" section in View mode. *}
