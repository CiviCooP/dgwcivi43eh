...
<h3>{ts}Case Summary{/ts}</h3>
	    ...
	    {if not $multiClient}
    		 ...
    		 {foreach from=$caseRoles.client item=client}
               ...
               {if $client.contact_id}
					...
					{crmAPI var="naw" entity="Contact" action="get" version="2" contact_id=$clientID}
    ...
    <table class="form-layout">
        ...
        {* CoreCorp12 translate print button *}    
		...
		<span class="crm-button"><div class="icon print-icon"></div><input type="button"  value="Dossieroverzicht afdrukken" name="case_report_all" onclick="printCaseReport( );"/></span>
		{* end CoreCorp12 *}
    ...
    <table class="report-layout">
        ...
        <tr>
          ...
          {if $relId neq 'client' and $hasAccessToAllCases}
            <td id ="edit_{$rowNumber}">
				...
				{* CoreCorp 12 translate pencil actions edit and delete *}
            	...
            	<a href="#" title="bewerken rol" onclick="createRelationship( {$row.relation_type}, {$row.cid}, {$relId}, {$rowNumber}, '{$row.relation}' );return false;">
            	...
            	<div class="icon delete-icon" title="verwijderen contact van rol"></div>
            	{* end CoreCorp12 *}
            	</a>
        ...
        {foreach from=$caseRoles item=relName key=relTypeID}
         {if $relTypeID neq 'client'} 
	       ...
	       <a href="#" title="rol bewerken" onclick="createRelationship( {$relTypeID}, null, null, {$rowNumber}, '{$relName}' );return false;">
	       {* end CoreCorp12 *}
